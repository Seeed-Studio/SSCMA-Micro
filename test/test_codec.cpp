...
TEST(EncoderTest, EncoderJSONTEST) {
    ...
    classes.push_back({0.86, 0});
    classes.push_back({0.87, 1});
    EXPECT_EQ(encoder.write(classes), MA_OK);
    std::vector<ma_point_t> points;
    points.push_back({0.15, 0.32, 0.89, 1});
    points.push_back({0.16, 0.33, 0.90, 1});
    EXPECT_EQ(encoder.write(points), MA_OK);
    std::vector<ma_bbox_t> bboxs;
    bboxs.push_back({0.1, 0.2, 0.3, 0.4, 0.86, 0});
    bboxs.push_back({0.2, 0.3, 0.4, 0.5, 0.87, 1});
