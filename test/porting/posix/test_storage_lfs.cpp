...
protected:
    void SetUp() override {
        storage = new ma::StorageLfs("test_data.lfs");
    }

    void TearDown() override {
        std::remove("test_data.lfs");
    }
...
TEST_F(StorageLfsTest, LongKVFuzz) {
    {
        ...
        int32_t v      = 42;
        EXPECT_EQ(storage->set(lk, v), MA_OK);
        int32_t rv;
        EXPECT_EQ(storage->get(lk, rv), MA_OK);
        EXPECT_EQ(rv, v);
    }

    {
        std::string lk =
            "abcdefghijklmno//////////////////////d/w/e/d/w/e/d/w/ed/w/e/fw/ef/"
...

    {
        ...
        int32_t v2 = 42;
        EXPECT_EQ(storage->set(lkv, v2), MA_OK);
        int32_t rv2;
        EXPECT_EQ(storage->get(lkv, rv2), MA_OK);
        EXPECT_EQ(rv2, v2);
