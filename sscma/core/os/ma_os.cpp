...
// RingBuffer class implementations
RingBuffer::RingBuffer(size_t size) noexcept {
    rb_ = ma_ringbuf_create(size);
}

RingBuffer::~RingBuffer() noexcept {
    if (rb_) {
        ma_ringbuf_delete(rb_);
    }
}
...
size_t RingBuffer::size() const {
    return ma_ringbuf_size(rb_);
}

void RingBuffer::clear() {
    if (rb_) {
        ma_ringbuf_clear(rb_);
    }
}

size_t RingBuffer::available() const {
    return ma_ringbuf_available(rb_);
}

size_t RingBuffer::free() const {
    return ma_ringbuf_free(rb_);
}

size_t RingBuffer::write(const void* data, size_t size, uint32_t timeout) {
    return ma_ringbuf_write(rb_, data, size, timeout);
}

size_t RingBuffer::read(void* data, size_t size, uint32_t timeout) {
    return ma_ringbuf_read(rb_, data, size, timeout);
}

RingBuffer& operator<<(RingBuffer& rb, char data) {
    rb.write(&data, 1, 0);
    return rb;
}
...
char RingBuffer::operator[](size_t index) {
    char data;
    read(&data, 1, 0);
    return data;
}
